		...
		return {
			...
			*/
			...
			shareShow : function(){
				...
				//	bts=[{title:"新浪微博"},{title:"微信好友"},{title:"信朋友圈"},{title:"分享到QQ"}];
				var ids = [{id:"weixin",ex:"WXSceneSession"},{id:"weixin",ex:"WXSceneTimeline"}];
				var	bts = [{title:"微信好友"},{title:"微信朋友圈"}];
			...
			shareShowPage : function(){
				var shareArea = document.getElementById('sharebox');
				var ids 	  = [
									{id:"weixin",title:"微信好友",ex:"WXSceneSession",icon:'weixin'},
									{id:"weixin",title:"微信朋友圈",ex:"WXSceneTimeline",icon:'pengyouquan'}
								];
				//添加元素
			  	for (var i = 0; i < ids.length; i++) {
			  		if(typeof ids[i] == 'object'){
						...
						btn.setAttribute('share_type', item.id);
						btn.setAttribute('ex', item.ex);
						btn.style.backgroundImage = 'url("../../../images/login/'+item.icon+'.png")';
						shareArea.appendChild(btn);
			  		}
			    };
			    var bntObj = document.querySelectorAll('.oauth-btn');
			    for(var i = 0;i < bntObj.length;i++){
			    	bntObj[i].addEventListener('tap', function(event) {
						var id = this.getAttribute('share_type');
			...
			shareMessage : function(s, ex){
				...
				var sendMessage = function(){
					...
					s.send(msg, function(){
						...
						//console.log("分享到\""+s.description+"\"成功，返回应用 ");
						//分享给qq好友，微信好友如果不返回应用，无法监听到分享成功回调
						params.status  = 0;
						if(callback){
							successCallback();
						}
	...
	var successCallback = function(){
		BDF.api('/user/Fund/activeFund', {
                'type': 'POST',
                'data': {
                    "fund_id": ajax_params.list_id
                }
			}, function (result) {
				if(result.status == '0'){
					var obj = document.getElementById(ajax_params.list_id);
					BDF.removeClass(obj, "dis-shared");
				}
		});
	}